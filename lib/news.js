...
function rssOgeleri(xml) {
  ...
  while ((m = re.exec(xml))) {
    ...
    out.push({ baslik: htmlCoz(baslik).trim(), kaynak: htmlCoz(kaynak).trim() });
  }
  return out;
}
...
async function haberGetir(symbol, ad) {
  ...
  for (const liste of sonuclar) {
    for (const h of liste) {
      ...
      if (!h.baslik || gorulen.has(anahtar)) continue;
      gorulen.add(anahtar);
      haberler.push(h);
      if (haberler.length >= 12) break;
    }
    if (haberler.length >= 12) break;
  }
  if (!haberler.length) return null;
  return { ...tonHesapla(haberler), haberler, kaynak: 'Google Haberler + Yahoo Finance' };
}
